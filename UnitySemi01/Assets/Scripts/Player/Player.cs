...
public class Player : MonoBehaviour
{
	...
	// PLのヒットポイントを直接操作。プラスが回復
	public void addPLHitPoint(int value)
	{
		hitBase.addHP(value);
		Color col;
		...
		int result;
		if (value > 0){
			result = (int)HitBase.RESULT.RECOVERY;
			col = new Color(0.5f, 1, 0.5f, 1);
		}
		else if (value == -DIE_DAMAGE){
			result = (int)HitBase.RESULT.DIE;
			col = new Color(1, 0, 0, 1);
		}
		else
		{
			result = (int)HitBase.RESULT.DAMAGE;
			col = new Color(1, 0, 0, 1);
		}

		hitBase.bootDamageText(transform.position,
			value.ToString(),
			col
		);

		// リアクションさせる
		setPLReaction(result);
	}


	// 外部呼出しメソッド
	public void setPLReaction(int result)
	{
		switch (result) {
			case (int)HitBase.RESULT.NONE:
				return;

			case (int)HitBase.RESULT.DAMAGE:
				// オーディオを再生
				SeManager.Instance.Play("damage1");
				this.StartCoroutine("Damage");
				break;

			case (int)HitBase.RESULT.DIE:
				// オーディオを再生
				this.StartCoroutine("Die");
				break;

			case (int)HitBase.RESULT.RECOVERY:
				// 回復している
